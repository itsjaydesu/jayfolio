...
function generateBreadcrumb(pageName, content, siteUrl) {
  ...
  if (content?.title) {
    const titleText = getLocalizedContent(content.title, 'en') || String(content.title || '').trim();
    items.push({
      '@type': 'ListItem',
...
function generateContentStructuredData(content, contentType, seoData) {
  const titleText = getLocalizedContent(content.title, 'en') || String(content.title || '').trim();
  const summaryText = getLocalizedContent(content.summary, 'en');
  const descriptionText = getLocalizedContent(content.description, 'en');
  const baseData = {
    '@context': 'https://schema.org',
    name: titleText,
    description: summaryText || descriptionText,
