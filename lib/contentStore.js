...
export async function readEntries(type) {
  ...
  return [...entries].sort((a, b) => {
    ...
    const aTitle = getLocalizedContent(a?.title, 'en') || '';
    const bTitle = getLocalizedContent(b?.title, 'en') || '';
...
export function hasTranslation(entry, language = 'en') {
  ...
  if (language === 'en') {
    return Boolean(getLocalizedContent(entry.title, 'en')?.trim());
  }

  const title = typeof entry.title === 'object' ? entry.title?.[language] : null;
  const summary = typeof entry.summary === 'object' ? entry.summary?.[language] : null;
  const content = typeof entry.content === 'object' ? entry.content?.[language] : null;

  return Boolean(
    (typeof title === 'string' && title.trim()) &&
    ((typeof summary === 'string' && summary.trim()) || (typeof content === 'string' && content.trim()))
  );
}
