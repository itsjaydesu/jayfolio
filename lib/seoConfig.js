...
export async function getContentSeoData(type, content) {
  ...
  // Localize primary strings to avoid "[object Object]" in titles/descriptions
  const localizedTitle = getLocalizedContent(content.title, 'en') || String(content.title || '').trim();
  const localizedSummary = getLocalizedContent(content.summary, 'en');
  const localizedDescription = getLocalizedContent(content.description, 'en');

  const title = template.titleTemplate
    ? template.titleTemplate.replace('%s', localizedTitle)
    : (localizedTitle ? `${localizedTitle} | ${global.siteName || 'jayfolio'}` : global.defaultTitle);

  const baseDesc = localizedSummary || localizedDescription || global.defaultDescription;
  const description = template.descriptionTemplate
    ? template.descriptionTemplate.replace('%s', baseDesc)
    : baseDesc;
